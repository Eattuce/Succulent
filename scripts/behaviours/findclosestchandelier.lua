...
FindClosestChandelier = Class(BehaviourNode, function(self, inst, see_dist, safe_dist, tags, exclude_tags, one_of_tags)
    BehaviourNode._ctor(self, "FindClosestChandelier")
    self.inst = inst
    self.targ = nil
    self.see_dist = see_dist
    self.safe_dist = safe_dist
    self.lastchecktime = 0
    self.tags = {"chandelier"}
...
function FindClosestChandelier:DBString()
    return string.format("Stay near target %s", tostring(self.targ))
end

function FindClosestChandelier:Visit()
...
function FindClosestChandelier:PickTarget()
    ...
    if self.inst ~= nil then
        self.inst:PushEvent("linkchandelier", {home = self.targ})
    end
